...
#SBATCH --ntasks=1
#SBATCH --mem=120gb
#SBATCH --time=23:59:00 # 1 day max on tamia
#SBATCH --output=/scratch/s/spapais/ForeSight/logs/%x-%j.log
...
DATA_DIR=/home/s/spapais/links/projects/aip-swasland/datasets/nuscenes/
WORK_DIR=/scratch/s/spapais/ForeSight/work_dirs
WANDB_PERSIST_DIR=/scratch/s/spapais/ForeSight/wandb
CMD=${@:-bash}

# Load env if needed (e.g. when submitted via non-interactive SSH)
[[ -f ~/.bashrc ]] && source ~/.bashrc

# apptainer available via CVMFS full path (not exposed as a module on tamia)
...
--bind=/scratch/s/spapais/ForeSight/ckpt:/workspace/ForeSight/ckpt \
