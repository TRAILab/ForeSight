    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_tpd_fuseA',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        det_head=dict(
            ...
            # ---- Fusion: gated_blend ----
            # Per-slot α-gate over [w_feat, main_feat[:num_ti]] → blend →
            # [norm, ffn, norm, refine] on the temporal subset. Refine
            # produces fused (anchor, cls, qt, vis, rel) for slots
            # [0:num_ti]; non-temporal slots [num_ti:num_anchor] inherit
            # main's last refine.
            fusion_strategy="gated_blend",
            fusion_order=["norm", "ffn", "norm", "refine"],
            fusion_ffn=dict(
                type="AsymmetricFFN",
                in_channels=embed_dims,
                pre_norm=dict(type="LN"),
