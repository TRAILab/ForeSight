    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_ptnomapdn_decoder6_planwp_evalmatchmode',),
            interval=50)
    ],
)
load_from = None
...
# ================== pretrained model ========================
# Stage-1 `nomap_dn`: denoising queries, NO map head.
# Matched partner is `dn` (ckpt/sparsedrive_stage1_dn.pth), which is
# byte-identical to apollo 8gpu_noflash_dn/iter_43900.pth. Both sides:
# 8-GPU / batch 64 / 100 epochs (iter_43900). The ONLY difference is the
# map head -> this is the clean single-flag stage-1 map ablation.
# Comparator: `dn` -> headline = 0.3578 / 0.053% (K3408313, single seed).
load_from = 'ckpt/sparsedrive_stage1_nomap_dn.pth'
