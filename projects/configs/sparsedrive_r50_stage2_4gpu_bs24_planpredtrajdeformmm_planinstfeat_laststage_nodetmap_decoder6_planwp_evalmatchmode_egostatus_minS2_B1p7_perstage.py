    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='Anon',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_egostatus_minS2_B1p7_perstage',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            # B1.7: conflict sampler reads image features at PLAN trajectory
            # waypoints + 7-point ±0.45 m footprint expansion (corridor coverage).
            # No detection forward pass needed; per-mode-restricted aggregation.
            # B1.7_perstage: re-sample image features at every refine stage
            # using the latest plan_anchor (cumsum of plan_reg.detach()) instead
            # of the static k-means prior, so collision evidence stays aligned
            # with the current trajectory hypothesis at each decoder stage.
            conflict_input='image_at_plan',
            conflict_resample_per_stage=True,
