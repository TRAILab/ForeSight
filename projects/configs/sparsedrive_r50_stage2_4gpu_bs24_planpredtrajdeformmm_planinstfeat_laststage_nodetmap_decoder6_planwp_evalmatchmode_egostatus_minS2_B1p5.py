    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_egostatus_minS2_B1p5',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            # B1.5: conflict sampler reads image features at PLAN trajectory
            # waypoints (no detection forward pass needed). Single point per
            # waypoint; per-mode-restricted aggregation in the loss.
            ...
            conflict_image_sampler=dict(
                ...
                kps_generator=dict(
                    type="SparseBox3DKeyPointsGenerator",
                    num_learnable_pts=6,
                    fix_scale=[[0, 0, 0]],  # B1.5: single point per waypoint
                ),
            ),
        ),
    ),
