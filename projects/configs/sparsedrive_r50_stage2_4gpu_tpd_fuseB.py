    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_tpd_fuseB',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        det_head=dict(
            ...
            # ---- Temporal Prior Decoder (TPD) ----
            # Pre-decoder warmup pass on the temporal subset of queries.
            # `temp_gnn` cross-attends cached temporal det features (Q) to the
            # previous-frame motion-aggregated agent feature (K/V), positionally
            # indexed by the predicted top-1 trajectory endpoint. No-op on the
            # very first frame; main-decoder grad still flows via the closing
            # `refine`.
            temporal_warmup_order=[
                "temp_gnn", "norm", "ffn", "norm", "refine",
            ],
            warmup_temp_graph_model=dict(
                type="MultiheadFlashAttention",
                embed_dims=embed_dims if not decouple_attn else embed_dims * 2,
                num_heads=num_groups,
                batch_first=True,
            ...
            # ---- Fusion: concat_fuse (B2 N=1) ----
            # Concat [main_feat (num_anchor); w_feat (num_temp_instances)]
            # → [temp_gnn (cross-attn to motion cache, self-attn fallback),
            # norm, ffn, norm, refine]. Refine produces fused outputs over
            # the full concat; final det_output uses the first num_anchor.
            fusion_strategy="concat_fuse",
            fusion_order=["temp_gnn", "norm", "ffn", "norm", "refine"],
            fusion_temp_graph_model=dict(
                type="MultiheadFlashAttention",
                embed_dims=embed_dims if not decouple_attn else embed_dims * 2,
                num_heads=num_groups,
                batch_first=True,
