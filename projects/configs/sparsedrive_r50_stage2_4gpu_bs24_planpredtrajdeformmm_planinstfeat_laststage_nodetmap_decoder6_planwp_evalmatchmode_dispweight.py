    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_dispweight',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            motion_sampler=dict(
                type="MotionTarget",
                # GT-displacement weighting: down-weight stationary/short
                # trajectories, up-weight long/turning trajectories. Mitigates
                # the L1 loss being dominated by trivial stationary cases.
                # min_w=0.5 / max_w=2.0 / scale_m=10.0 keeps loss magnitude
                # bounded to [0.5x, 2x] of original; scale_m=10m means a 10m
                # endpoint displacement (≈ urban driving over 3 s) → weight 1.0.
                displacement_weight=dict(min_w=0.5, max_w=2.0, scale_m=10.0),
            ),
            ...
            planning_sampler=dict(
                ...
                ego_fut_ts=ego_fut_ts,
                ego_fut_mode=ego_fut_mode,
                # Same displacement weighting on the planning loss.
                displacement_weight=dict(min_w=0.5, max_w=2.0, scale_m=10.0),
            ),
            plan_loss_cls=dict(
                type='FocalLoss',
