# ================ base config ===================
# Option 3: joint stage-1 with GT→pred curriculum for motion/plan.
# Motion/planning receives GT boxes for the first gt_det_warmup_iters iterations,
# then switches to predicted outputs for the remainder of training.
# 1172 iters/epoch on 8 GPUs (bs=24 total) → warmup_iters=58600 ≈ first 50 epochs use GT.
version = 'mini'
version = 'trainval'
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage1_8gpu_noflash_joint_curriculum',),
            interval=50)
    ],
)
load_from = None
...
gt_det_warmup_iters = 58600  # ~50 epochs on 8 GPUs (1172 iters/epoch at bs=24)

task_config = dict(
    with_det=True,
    with_map=True,
    with_motion_plan=True,
    gt_det_warmup_iters=gt_det_warmup_iters,  # GT oracle for first ~50 epochs, then predicted
)
