...
# R101 at 1408x512 is ~4x the pixels of the r50 baseline and the Trillium wall
# limit is 24h, so save intermediate checkpoints to make the run resumable.
# Evaluation stays on checkpoint_epoch_interval (end of training only).
checkpoint_save_epoch_interval = 2

checkpoint_config = dict(
    interval=num_iters_per_epoch * checkpoint_save_epoch_interval
)
log_config = dict(
    interval=51,
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r101_stage2_4gpu_bs32_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_egostatus_minS2',),
            interval=50)
    ],
)
load_from = None
