# Step 1 of the planner redesign: clean PlanningOnlyHead skeleton with the
# same per-stage layout as the minS2 ego-only baseline. Goal is parity with
# `..._egostatus_minS2.py`. Architecture changes (joint self-attn, etc.) land
# in step 2; this config keeps temp_gnn / deformable / ffn / refine × 6 and
# the conflict head reading image features at plan-anchor BEV positions.
    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='Anon',
                name='sparsedrive_r50_stage2_4gpu_bs24_planneronly_skeleton_noegostatus',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        det_head=dict(
            ...
            loss_cls=dict(
                type="FocalLoss",
                use_sigmoid=True, gamma=2.0, alpha=0.25,
                loss_weight=0.0,
            ),
            loss_reg=dict(
        ...
        motion_plan_head=dict(
            ...
            operation_order=(
                ["temp_gnn", "deformable", "ffn", "refine"] * 6
            ),
            temp_graph_model=dict(
            ...
            refine_layer=dict(
                type="PlanningOnlyRefinementModule",
                embed_dims=embed_dims,
                ego_fut_ts=ego_fut_ts,
                ego_fut_mode=ego_fut_mode,
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            # Freeze backbone + perception heads (Stream E pattern). Only the
            # planner trains. Det/map cls + reg losses are zeroed above.
