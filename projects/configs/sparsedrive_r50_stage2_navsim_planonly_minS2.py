    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_navsim_planonly_minS2',),
            interval=50)
    ],
)
load_from = None
...
num_sample = 20
# NavSim future horizon: 8 steps @ 0.5s = 4s. Driving cmd is 4-dim (left/straight/right/unknown).
fut_ts = 16
fut_mode = 6
ego_fut_ts = 8
ego_fut_mode = 6
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            # NavSim plan anchors must be regenerated on navtrain (4 cmds × 6 modes × 8 steps).
            # Drop into data/kmeans/ before running training. See reports/2026_04_29_navsim_setup.md.
            motion_anchor=f'data/kmeans/kmeans_motion_navsim_{fut_mode}.npy',
            plan_anchor=f'data/kmeans/kmeans_plan_navsim_{ego_fut_mode}_cmd{num_driving_cmds}.npy',
            # minS2: encode current ego status into the planner's mode query
            # via an MLP. Indices select acc_xy (0,1), rot_rate_z (5), vel_xy
            # (6,7) — the 5 channels populated from AgentInput in
            # SparseDriveFeatureBuilder.compute_features.
            plan_ego_status_encode_enable=True,
            plan_ego_status_indices=(0, 1, 5, 6, 7),
            # minS2: drop agent slots from the planner's queries. Consistent
            # with this config's planning-only setup (gt_agent_fut_* are empty
            # placeholders, motion losses zeroed below — agent supervision
            # would require Stage-1 perception GT).
            ...
            # minS2: zero motion losses. With agent futures unwired
            # (gt_agent_fut_* empty), motion supervision is meaningless and
            # only contributes noise via cross-attn back to the planner.
            ...
            planning_decoder=dict(
                ...
                ego_fut_mode=ego_fut_mode,
                num_driving_cmds=num_driving_cmds,
                # minS2: disable learned-soft rescoring; keep the (cheaper)
                # learned-hard path active. Reduces eval-time complexity for
                # PDM-Score without losing planning quality on nuScenes.
                use_rescore=False,
            ...
            # Tier 2 cycle 3 attempted to port minS2's full architectural
            # block (planning/motion_cumulative_refinement, planning_deformable
            # + sub-flags, motion_deformable_multimode). At 1 epoch this
            # regressed PDMS 0.6489 → 0.5670 — the added per-layer/per-waypoint
            # learnable params need more than 1 epoch to settle. Reverted;
            # may revisit at 10-epoch scale.
        ),
    ),
)
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            # minS2: full freeze of backbone + perception heads. With
            # ego_only_planning=True and motion losses zeroed, effectively
            # only motion_plan_head's planner sub-head trains. Replicates
            # the freeze pattern that landed nuScenes' best stage-2 number
            # (L2=0.3563 on minS2).
