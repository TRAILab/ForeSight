    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_egostatus_s2nopercep',),
            interval=50)
    ],
)
load_from = None
    ...
    head=dict(
        ...
        det_head=dict(
            ...
            loss_cls=dict(
                ...
                gamma=2.0,
                alpha=0.25,
                loss_weight=0.0,  # s2nopercep: zero det cls
            ),
            loss_reg=dict(
                type="SparseBox3DLoss",
                loss_box=dict(type="L1Loss", loss_weight=0.0),  # s2nopercep
                loss_centerness=dict(type="CrossEntropyLoss", use_sigmoid=True, loss_weight=0.0),  # s2nopercep
                loss_yawness=dict(type="GaussianFocalLoss", loss_weight=0.0),  # s2nopercep
        ...
        map_head=dict(
            ...
            loss_cls=dict(
                ...
                gamma=2.0,
                alpha=0.25,
                loss_weight=0.0,  # s2nopercep: zero map cls
            ),
            loss_reg=dict(
                type="SparseLineLoss",
                loss_line=dict(
                    type='LinesL1Loss',
                    loss_weight=0.0,  # s2nopercep: zero map line
                    beta=0.01,
                ),
                num_sample=num_sample,
                roi_size=roi_size,
        ...
        motion_plan_head=dict(
            ...
            motion_loss_cls=dict(
                ...
                gamma=2.0,
                alpha=0.25,
                loss_weight=0.0  # s2nopercep: zero motion cls
            ),
            motion_loss_reg=dict(type='L1Loss', loss_weight=0.0),  # s2nopercep
