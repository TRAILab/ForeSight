    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage1_8gpu_noflash_aux2d_dsegv2',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    dense_seg_branch=dict(
        ...
        class_names=(
            ...
            # v2 polygon classes — planning-relevant filled-region supervision
            "drivable_area", "walkway", "stop_line",
        ),
        loss_weight=1.0,
    ),
    aux_2d_head=dict(
...
train_pipeline = [
    ...
    dict(
        type="GenerateDenseSegMask",
        downsample=strides[:num_depth_layers],
        # dseg v2 — explicit class_specs spelling out 9 channels:
        # 3 polyline + 3 agent + 3 polygon (planning-relevant map regions).
        class_specs=[
            ...
            dict(name="drivable_area", source="map_polygon",  layer="drivable_area"),
            dict(name="walkway",       source="map_polygon",  layer="walkway"),
            dict(name="stop_line",     source="map_polygon",  layer="stop_line"),
        ],
    ),
    dict(
        type='VectorizeMap',
...
data_basic_config = dict(
    ...
    # dseg v2 — live-extract polygon layers (drivable_area, walkway, stop_line).
    # The saved map_annos pkls only contain LineStrings; NuscMapExtractor
    # is loaded once per worker and called per sample to fetch polygon GTs.
    polygon_geom_layers=("drivable_area", "walkway", "stop_line"),
