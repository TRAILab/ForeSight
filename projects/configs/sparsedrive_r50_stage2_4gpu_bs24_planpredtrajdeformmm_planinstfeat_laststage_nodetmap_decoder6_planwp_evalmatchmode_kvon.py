    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_kvon',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            # kvon: single-flag K/V-on control for the headline. The parent
            # config nulls the planner's `gnn`/`cross_gnn` ops via
            # `skip_perception_kv=True` and starves `cross_gnn` via
            # `num_map=0`. Here both are restored to the SparseDrive stage-2
            # baseline setting, so det + map tokens enter the planning
            # decoder's cross-attention at inference. Everything else —
            # learned scorer, egostatus, decoder6, planwp, laststage
            # instfeat, stage-1 init, bs/lr — is held fixed against the
            # 3-seed anchor (0.3692 / 0.0400%).
            num_det=50,
            num_map=10,
