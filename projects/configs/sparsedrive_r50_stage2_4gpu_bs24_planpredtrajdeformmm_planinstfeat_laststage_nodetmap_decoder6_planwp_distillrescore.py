    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='Anon',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_distillrescore',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            # Distill HierarchicalPlanningDecoder.rescore() collide flag into
            # plan_cls during training. At inference, `use_rescore=False` and
            # the planner's own logits encode the constraint without the
            # explicit -999 mask. det_output (from the perception head, still
            # trained as backbone supervisor) is the supervision source — does
            # not require det K/V into the planner.
            plan_distill_rescore_enable=True,
