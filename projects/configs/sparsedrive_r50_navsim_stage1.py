    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_navsim_stage1',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        # Stage-1 navsim: map_head produces real polylines that the planner
        # cross-attends over via num_map=10, so we need it at eval too.
        # eval_skip_map stays at the default False.
        det_head=dict(
            type="Sparse4DHead",
        ...
        motion_plan_head=dict(
            ...
            # Keep the ego-status MLP encoder from minS2 — works equally well
            # with stage-1 supervision (real ego_status channels still flow in).
            plan_ego_status_encode_enable=True,
            plan_ego_status_indices=(0, 1, 5, 6, 7),
            # Stage-1: real per-agent motion GT is now wired, so agent slots
            # in the planner queries carry useful information. Drop minS2's
            # ego_only_planning=True so the planner can cross-attend over them.
            embed_dims=embed_dims,
            ...
            # Stage-1: real per-agent motion GT wired (cross-frame track walk
            # producing (N, 16, 2) deltas + masks). Restored loss weights to
            # nuScenes minS2 reference values.
            ...
            num_det=50,
            # Stage-1: planner cross-attends over real det + map outputs.
            num_map=10,
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            # Stage-1 navsim: backbone trains at 10% lr (mirrors nuScenes
            # stage-2 baseline). Perception heads + planner train at 1.0×.
