    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='Anon',
                name='sparsedrive_r50_stage2_4gpu_bs24_isolated',),
            interval=50)
    ],
)
...
task_config = dict(
    with_det=True,       # kept for planner infrastructure (anchor_encoder, instance_bank)
    with_map=False,      # architecturally removed (Condition B-style)
...
model = dict(
    ...
    depth_branch=dict(  # for auxiliary supervision only
        ...
        loss_weight=0.0,  # isolated: depth aux off
    ),
    head=dict(
        ...
        det_head=dict(
            ...
            loss_cls=dict(
                ...
                gamma=2.0,
                alpha=0.25,
                loss_weight=0.0,  # isolated: det cls supervision off
            ),
            loss_reg=dict(
                type="SparseBox3DLoss",
                loss_box=dict(type="L1Loss", loss_weight=0.0),  # isolated: det box supervision off
        ...
        motion_plan_head=dict(
            ...
            motion_loss_cls=dict(
                ...
                gamma=2.0,
                alpha=0.25,
                loss_weight=0.0  # isolated: motion cls supervision off
            ),
            motion_loss_reg=dict(type='L1Loss', loss_weight=0.0),  # isolated: motion reg supervision off
