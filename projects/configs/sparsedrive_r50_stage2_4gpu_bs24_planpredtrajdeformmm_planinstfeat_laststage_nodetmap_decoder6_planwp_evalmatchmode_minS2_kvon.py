    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_minS2_kvon',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            plan_ego_status_encode_enable=False,  # no-ego regime
            ...
            # kvon: single-flag K/V-on control for minS2. The parent nulls the
            # planner's `gnn`/`cross_gnn` ops via `skip_perception_kv=True` and
            # starves `cross_gnn` via `num_map=0`; both are restored here so
            # the ego query cross-attends to det + map tokens at inference.
            # Note `ego_only_planning=True` is retained (Stream C), so this
            # measures ego-query-to-perception-token K/V specifically. The det
            # and map heads are frozen at `lr_mult=0.0` with their losses
            # zeroed, so the tokens come from the stage-1 perception heads.
            num_det=50,
            num_map=10,
            ...
            # minS2: conflict head reads image features at det-anchor BEV
            # positions (Stream B1 / Stream C); det/motion queries dropped.
