    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_bs24_ptdnrot3daux2p5d_planpredtrajdeformmm_planinstfeat_laststage_nodetmap_decoder6_planwp_evalmatchmode_egostatus_minS2_2a_aux',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        motion_plan_head=dict(
            ...
            # 2a-aux: SceneQueryDecoder + detection-like aux loss on scene
            # queries against ego-interacting GT subset (within 20m of ego in
            # next 3s). Hungarian-matched focal cls + L1 (x, y) box loss.
            conflict_input='image_at_scene_query',
            scene_query_aux_loss_enable=True,
            ...
            conflict_scene_query_decoder=dict(
                type='SceneQueryDecoder',
                embed_dims=embed_dims,
                num_queries=50,
                anchor_path='ckpt/kmeans_ego_interact_K50_d20m_T3s.npy',
                num_layers=2,
                num_heads=num_groups,
                ffn_dim=512,
                dropout=drop_out,
