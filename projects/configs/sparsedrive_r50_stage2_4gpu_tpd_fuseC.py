    ...
    hooks=[
        ...
        dict(type='WandbLoggerHook',
            init_kwargs=dict(
                entity='trailab',
                project='ForeSight',
                name='sparsedrive_r50_stage2_4gpu_tpd_fuseC',),
            interval=50)
    ],
)
load_from = None
...
model = dict(
    ...
    head=dict(
        ...
        det_head=dict(
            ...
            # ---- Fusion: pred_ensemble ----
            # No new params. Per-slot sigmoid-confidence-weighted blend of
            # (anchor, cls, qt, vis, rel) for slots [0:num_ti] between TPD's
            # warmup refine output and main's last refine output. Trained
            # so motion-loss gradients flow through both branches via the
            # blend.
            fusion_strategy="pred_ensemble",
        ),
        map_head=dict(
            type="Sparse4DHead",
            cls_threshold_to_reg=0.05,
