...
| `_ptaux2d_dseg_decoder6_planwp_evalmatchmode_egostatus_minS2_B1p6` | 1 | 0.3706 | 0.048% | - | - | - | - | 0.5354 | - | 0.3299 | **(K3406571)** dseg stage-1 × minS2 + B1.6 conflict sampler. Stronger stage-1 recovers most of the CR cost: dseg+B1.6 CR=0.048% vs default-S1+B1.6 CR=0.064% (~0.016 pp recovery). **Best fully detection-free path**: ties minS2 on CR within noise (0.046% vs 0.048%) while removing the last detection-forward dependency at inference. |
...
- **Detection-free path lands cleanly with stronger S1 (`_ptaux2d_dseg_minS2_B1p6`, K3406571): L2=0.3706 / CR=0.048%.** dseg stage-1 paired with B1.6 conflict sampler recovers most of the CR cost of dropping det-anchor positions (default-S1 + B1.6 retrain K3406567: 0.064% → dseg + B1.6: 0.048%, ~0.016 pp recovery). CR now ties minS2 (0.046%) within noise while the inference path requires *no* detection or map forward pass. **The paper's strongest claim — "fully detection/map-free at inference, matching K/V-on baselines on both L2 and CR" — is now end-to-end demonstrated on a single config.** B1.8 (K3406568) and B1.9 (K3406570) without dseg both elevate CR (~0.07%) like B1.6 default-S1, so the recovery is specifically the dseg×B1.6 pairing. Full B1.6 retrain alone (K3406567) didn't recover CR vs eval-only — the recovery is in the stage-1, not the retrain.
