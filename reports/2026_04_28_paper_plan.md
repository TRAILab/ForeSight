...
- [ ] **2a_aux × strongest-S1 batch SUBMITTED 2026-05-05 (Killarney, 3 jobs, 12h slot).** Tests learned scene-query conflict head (Option 2a, `image_at_scene_query` + ego-interaction aux supervision) against the locked B1.6 numbers, paired with the strongest reproduced S1 inits. K3434003 (`ptdnrot3daux2p5d × minS2 × 2a_aux`), K3434006 (`ptjoint_nodetach × minS2 × 2a_aux`), K3434011 (`_minS2_2a_aux` seed-1, default S1 reproduction of K3399643 0.3634/0.040%). Existing single-seed default-S1 2a_aux ties headline on both metrics; batch tests if (a) it reproduces, (b) strongest S1 makes 2a_aux beat B1.6 on CR (B1.6 elevated by 0.014–0.024 pp vs minS2), (c) joint-S1 ultra-low CR survives the learned-anchor swap. Mechanism for paper: K=50 BEV anchor queries refined per-scene via deformable cross-attn to image features, supervised against ego-interacting GT subset (within 20 m / 6 future steps, cls_weight=1.0, box_weight=2.0), then conflict head reads image features at the refined positions — the principled answer to "learn a planning-critical downsampling of scene tokens conditioned on ego planning needs" (`2026_04_22_nuerips26_paper.md`).
...
- **Tamia and rorqual were missing commit `95db051` (SparseBox3DKeyPointsGenerator dtype fix); both are now pulled.** Pre-fix `_egostatus` results were invalid (rorqual 0.131%, tamia 0.243% vs expected ~0.040%). **Post-pull confirmation: Tamia `_egostatus` (272885, 2026-05-03 PM) lands canonical at L2=0.3705 / CR=0.032%** — the prior 0.243% "deterministic Tamia offset" was the missing-fix artifact. Rorqual post-pull `_egostatus` reads pending (11262594/152 crashed at eval on missing `data/nuscenes/lidarseg/v1.0-trainval`; same lidarseg-mount issue also hit Fir 38334912/38334913/38334420 today — separate infra issue from the dtype fix; doesn't affect training).
...
| 6 | **`aux2d_dsegv2`** (dseg v1 + 3 polygon channels: drivable_area, walkway, stop_line) | **landed** (S1 T485018: NDS=0.5426, mAP_normal=0.5647) | 0.3687 / 0.036% (K3446935) | ties default S1 within noise on L2; CR −0.004 pp. **No measurable lift over dseg v1** (0.3621/0.043% K3396708). Polygon-class supervision didn't unlock further planning gain. No-ego transfer K3446936: 0.5083/0.070% — ties no-ego cluster (3-cluster mean 0.5197/0.068%). |
...
| 9 | `joint_nodetach` (`detach_perception=False`) | **landed** | **0.3679 / 0.029%** (K3412998) — **lowest CR of any S1 swap; ΔL2 within noise of 3-seed mean.** Validates the joint-S1 hypothesis after a year of negative Arm A/B results. NDS=0.5024 (slight perception trade-off). Single-seed; reproduction recommended. Pair with minS2 × B1.6 next. |
...
- B1.6/B1.7 eval-only evals (3401950, 3401952) tied L2 within noise but raised CR ~0.02–0.03 pp. **Full B1.6 retrain (K3406567): L2=0.3629 / CR=0.064%** — L2 marginally below headline mean (within noise), CR still elevated by ~0.024 pp vs minS2 (0.041%). New conflict-sampler variants: **B1.8 (K3406568): L2=0.3644 / CR=0.072%**; **B1.9 (K3406570): L2=0.3691 / CR=0.065%** — both tie L2 within noise, both elevate CR. **dseg×B1.6** seed-0 (K3406571): L2=0.3706/CR=0.048%; seed-1 (K3412422 --seed 1): L2=0.3651/CR=0.063%. **2-seed mean L2=0.3679 / CR=0.0555%** — L2 within noise of headline; CR ~0.015 pp above minS2 mean. **`_ptaux2d_dseg_minS2_B1p7` (K3413049): L2=0.3674 / CR=0.055%** ties dseg×B1.6 within noise (sampler choice = noise within minS2). **`_ptdnrot3daux2p5d_minS2_B1p6` (K3412959): L2=0.3622 / CR=0.059% / NDS=0.5636 / mAP_normal=0.3948** — new strongest S1 paired with B1.6. Planning ties dseg×B1.6 within noise; **perception metrics jump significantly** (NDS +0.029, mAP_normal +0.065 vs dseg×B1.6) — best candidate for paper headline if it reproduces. Single-seed; reproduction recommended.
