...
- [ ] **Stream C — `_decoder6_planwp_evalmatchmode_streamc`** (Killarney 3386494 crashed at startup; resubmitted as **3389115** 2026-05-01, training COMPLETED to iter 11679/11720 — essentially the full schedule — but eval crashed at startup with `KeyError: 'trajs_3d'`. The `ego_only_planning=True` flag strips agent slots whose `trajs_3d` output the eval pipeline still queries. Final ckpt intact; result not yet readable. Needs eval-time fix gating `trajs_3d` access on `ego_only_planning`). Combines the three confirmed K/V-off levers: `plan_ego_status_encode_enable=True` (egostatus), `conflict_input='image_at_det'` (Stream B1 imgconfdet), and `ego_only_planning=True` (drop agent slots from planner). First attempt 3386494 hit a size-0 tensor mismatch in `motion_blocks.py:190` (the conflict-head pairwise concat used `num_anchor` from `motion_query.shape[1]` which goes to 0 under `ego_only_planning=True`, while `agent_features.shape[1]` from `image_at_det` is still num_det=50). Fixed by sourcing the expand size from `agent_features.shape[1]` (commit `7261fa7`); the change is a no-op in every prior config since the two were equal. Tests the paper's strongest claim — **zero-dependence stage 2** at inference: planner reads only image features + ego state, conflict head reads only image features at det BEV positions, no agent queries in the planner's instance_feature. If CR holds within ~0.01 pp of evalmatchmode_egostatus (0.044%) and L2 within ~0.02 of 0.3720, this **is** the zero-dependence stage-2 architecture for the paper headline.
