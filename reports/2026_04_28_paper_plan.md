...
- **F1 (GT-displacement weight, simplest)**: per-sample weight = `clip(endpoint_displacement / scale_m, min_w, max_w)`. Stationary → `min_w`, fast turns → `max_w`. No new labels. Defaults `min_w=0.5, max_w=2.0, scale_m=10.0` keep loss magnitude in [0.5×, 2×] of original. **Implemented as `displacement_weight` flag on `MotionTarget` and `PlanningTarget`.** First config: `_decoder6_planwp_evalmatchmode_dispweight` (Killarney **3376749**, submitted 2026-04-30).
...
- [ ] **Seed-1 reproduction of `_decoder6_planwp_evalmatchmode`** (Killarney **3377724** + DGX **3702** as parallel hedge, submitted 2026-05-01). Same config, `--seed 1`. Killarney work_dir `..._evalmatchmode_seed1`; DGX work_dir `..._evalmatchmode_seed1_dgx`. DGX duplicated because the Killarney queue had ~8 PDs ahead and the CR=0.046% headline is the most acute paper risk. Whichever finishes first answers the question; cancel the loser to free its slot. Tests whether the CR=0.046% headline survives across seeds. The base `_decoder6_planwp` reproduced on L2 (0.5150 → 0.5100, mean 0.5125) but **CR did not** (0.068 → 0.087); the question here is whether the learned rescore head's CR win is seed-stable on top of the K/V-off architecture. Required before locking contribution 4 in the paper.
- [ ] **`_decoder6_planwp_evalmatchmode_egostatus`** (Killarney **3376689**, submitted 2026-04-30). Folds the anchor-capacity `egostatus` lever (L2 0.4988 → **0.3701** on the K/V-on `ppdeformmm_planifls` baseline) into the K/V-off paper headline stack. `plan_ego_status_encode_enable=True`, `plan_ego_status_indices=(0,1,5,6,7)` MLP-encodes current-frame ego state and broadcast-adds it to the plan_mode_query at init + each per-stage rebuild. If the L2 win transfers to the K/V-off architecture (anywhere from −0.05 to −0.13 vs evalmatchmode 0.5204), this becomes the new headline; if not, egostatus stays as a K/V-on-only result and contribution 4 remains pinned to evalmatchmode.
- [ ] **Stream B1 — `_decoder6_planwp_evalmatchmode_imgconfdet`** (Killarney **3376418**, submitted 2026-04-30). Replaces the conflict head's `agent_features` input with image features sampled at det-anchor BEV positions via a deformable aggregator (`conflict_input='image_at_det'`). First cut at the Stream B "decouple conflict head from agent features" beat. If CR holds within ~0.01pp of evalmatchmode 0.046%, the conflict head's per-agent token isn't load-bearing — Stream C (drop det/motion queries from the joint decoder) becomes viable. If CR regresses, fall back to B2/B3 (BEV occupancy readout).
...
- [ ] Stage-1 aux2d Trillium runs (`aux2d_dino` 472496, `aux2d_dseg` 472563) hit 24 h TIMEOUT at iter 70320 = **epoch 60/100** (1172 iters/epoch × 60 epochs; checkpoint_epoch_interval=20 saved at epochs 20, 40, 60). No eval yet. Resubmitted 2026-04-29 with `--resume-from iter_70320.pth` as **474792** (dino) and **474793** (dseg), 23:59 limit each (≈ 40 epochs ≈ 16 h remaining). **Resume update (2026-05-01):** both resumes also TIMED OUT at 23:59:00 — `dino` reached iter 89709/117200 (epoch 76.5, no save past epoch 60), `dseg` reached iter 77775/117200 (epoch 66.4, no save past epoch 60). Neither hit the next `checkpoint_epoch_interval=20` save point at epoch 80 (iter 93760). Second resume in flight as **476862** (dino) / **476863** (dseg), both PENDING on Trillium as of 2026-05-01 04:05 UTC. To finish epoch 100 from `iter_70320.pth`: needs ~14 more epochs ≈ 5.5 h training + ~0.5 h eval ≈ 6 h budget — well within 23:59 if scheduling gets it on a stable node.
...
- [x] Anchor-capacity batch (`2026_04_30_anchor_capacity.md`). 5 variants on `ppdeformmm_planifls` baseline + 1 no-rescore eval, submitted Killarney 2026-04-30 (3368761–3368766). Cluster-wide failure at 19:20 UTC killed mid-eval on jobs 3368764, 3368765 — checkpoints saved, eval-only resubmitted as 3376375/3376376; those two were mis-allocated at 4 GPUs and re-resubmitted on 2 GPUs (`--gres=gpu:l40s:2 --cpus-per-task=8 --mem=60G`) as **3377754** (`_endptnorm_mag_egostatus`) and **3377755** (`_modesspeedstrat_perbucketreg`).
  ...
  - **`shapeanchor_endptnorm_mag_egostatus`** (variant 3): eval pending as **3377754** (re-resubmit at 2 GPUs).
  - **`modesspeedstrat_perbucketreg`** (variant 4): eval pending as **3377755** (re-resubmit at 2 GPUs).
...
3. **Apollo egoonly stage-1: COMPLETED 2026-04-30 23:51 UTC.** 5-epoch run, eval at every checkpoint. Final (`iter_58600.pth`, epoch 5): **L2=0.6518 / obj_box_col=0.156% / NDS=0.5170 / mAP=0.3989 / mAP_normal=0.5667**. Per-checkpoint trajectory shows epoch 3 (`iter_35160.pth`) was *planning-best* (L2=0.6351 / 0.097%) — model regressed slightly on epochs 4–5. Stage-2 follow-up `_ptegoonly_ppdeformmm_planifls` queued on Killarney with `ckpt/sparsedrive_stage1_egoonly.pth` (= epoch-5 final ckpt) and the K/V-on `ppdeformmm_planifls` recipe (direct comparator to #7's `_ptnomapdnrot_ppdeformmm_planifls` at L2=0.5203). If planning lands within ~0.02 of #7, the "minimum-perception stage-1 still useful" claim is supported and the supervision-vs-interface decoupling argument extends from "stage-1 nomap suffices" to "stage-1 ego-only-planning + det+map suffices". If it regresses, retry with the planning-best epoch-3 ckpt (`iter_35160.pth`).
