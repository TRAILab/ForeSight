...
| **7** | **`stage1_nomap_dn_rotaug` ckpt + stage-2 *with* map** (no new stage-1 train; already have ckpt) | resolves backbone-shaping vs inference-path confound from prior nomap evidence | 1× stage-2 only ~12h | side study / appendix |
